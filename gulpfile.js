...
var specFiles = [
  ...
  'lib/tgi-interface-onsen2.spec.js',
...
gulp.task('_buildLib', function () {
  return gulp.src(libPackaging)
    .pipe(concat('tgi-interface-onsen2.js'))
    .pipe(gulp.dest('dist'))
    .pipe(rename('tgi-interface-onsen2.min.js'))
    .pipe(uglify())
    .pipe(gulp.dest('dist'));
});
...
gulp.task('_buildLibChunk', function () {
  return gulp.src(libFiles)
    .pipe(concat('tgi.interface.onsen2.chunk.js'))
    .pipe(gulp.dest('dist'));
});
...
gulp.task('_buildSpec', function () {
  return gulp.src(specFiles)
    .pipe(concat('tgi-interface-onsen2.spec.js'))
    .pipe(gulp.dest('dist'));
});
...
// Copy onsen2
gulp.task('copyonsen2', function () {
  return gulp.src(['node_modules/onsenui/**']).pipe(gulp.dest('dist/onsenui'));
});
...
gulp.task('default', ['copyMarked', 'copyonsen2',  'test']);
